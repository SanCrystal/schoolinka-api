...
export const createBlog = async (
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Blog was created successfully", data: blog });
  } catch (error) {
    return res
      .status(500)
...
export const getAllBlogs = async (
  ...
  try {
    ...
    if (query) {
      const blogSearched = await client.blogPost.findMany({
        where: {
          OR: [
            {
              title: {
      ...
      const sortedBlogs = blogSearched.sort(
        (
          postA: Pick<Blog, "title" | "text">,
          postB: Pick<Blog, "title" | "text">,
        ) => {
          const countQueryA =
      ...
      return res.status(200).json({
        message:
          blogSearched.length > 0
            ? "Blogs successfully retrieved"
            : "No Blog matched your query.",
        data: sortedBlogs,
      });
    }
...
export const getBlogByID = async (
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Blog was fetched successfully", data: blog });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Internal Server Error " + error.message });
...
export const updateBlogByID = async (
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Blog was updated successfully", data: blog });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Internal Server Error " + error.message });
...
export const deleteBlogByID = async (
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Blog was deleted successfully", data: blog });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Internal Server Error " + error.message });
